...
|---|---|---|
| `ci.yml` | every push to `dev` and `main`, every PR | `commits` (Conventional Commits check), `lint` (fmt + clippy `-D warnings`), `test`, `build-linux`, `build-windows`, `cross-windows` (the zigbuild cross-compile), `msrv` (1.88), `changelog-preview` |
| `release.yml` | every push to `main` | reads the version from `Cargo.toml`, exits cleanly if `v<version>` is already tagged, builds Linux and Windows, resolves the notes, tags, regenerates `CHANGELOG.md` and publishes the GitHub release |
| `schedule.yml` | Mondays 05:00 UTC (`0 5 * * 1`), or manual | the toolchain canary and the `nightly` rolling build from `dev` |
...
CI-clean and nothing more — no live game has seen it.
